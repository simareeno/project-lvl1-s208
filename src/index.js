...
const askQuestion = (question, correctAnswer, qMessage) => {
  if (qMessage) {
    console.log(`${qMessage} ${question}`);
  } else {
...
export default (welcomeMessage, getQaPair, qMessage) => {
  ...
  const startRound = (currentRound) => {
    ...
    const isCorrect = askQuestion(question, correctAnswer, qMessage); // ask question
    return isCorrect ?
